...
var Geeklist = function (options) {
  ...
  this.consumer = {
      key    : this.options.consumer_key || process.env.GEEKLIST_CONSUMER_KEY
    , secret : this.options.consumer_secret || process.env.GEEKLIST_CONSUMER_SECRET
  }

  if ((this.options.access_token || process.env.GEEKLIST_ACCESS_TOKEN) && (this.options.access_secret || process.env.GEEKLIST_ACCESS_SECRET)) {
    this.access = {
        token: this.options.access_token || process.env.GEEKLIST_ACCESS_TOKEN
      , secret: this.options.access_secret || process.env.GEEKLIST_ACCESS_SECRET
    }
  }

  this.usePin = this.options.usePin || false

  this.callback = this.options.callback

  this.apiServer = this.options.api_server || 'http://api.geekli.st'
  this.wwwServer = this.options.www_server || 'http://geekli.st'
